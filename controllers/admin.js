...
exports.getProductForm = (req, res, next) => {
    res.render('add-product', { name: 'Cill', path: '/admin/add-product', pageTitle: 'Add Product' });
}
...
exports.editProductPage = (req, res, next) => {
    Product.findById(req.params.prodId)
        .then(product => {
            res.render('edit-product', { product: product, path: '/', pageTitle: 'Edit Product', name: 'Cill' });
        }).catch(err => console.log(err));
...
exports.editProductPost = (req, res, next) => {
    ...
    Product.updateOne({ _id: req.body.id }, { $set: { title: req.body.title, imageURL: req.body.imageURL, price: req.body.price, stock: req.body.stock, description: req.body.description } })
        .then(result => {
